 ...
 * @deprecated
 * @since 1.12.18
 * @see \Aimsinfosoft\Base\Block\Adminhtml\System\Config\Information
 */
class GenerateInformationTab implements ObserverInterface
{
    ...
    {
        $src = $this->assetRepo->getUrl('Aimsinfosoft_Base::images/Aimsinfosoft_logo.svg');
        if ($this->moduleInfoProvider->isOriginMarketplace()) {
            $href = Item::MAGENTO_MARKET_URL;
        } else {
            $href = 'https://store.aimsinfosoft.com/' . $this->getSeoParams() . 'Aimsinfosoft_logo_' . $this->getModuleCode();
        }

        $html = '<a target="_blank" href="' . $href . '"><img class="Aimsinfosoft-logo" src="' . $src . '"/></a>';
